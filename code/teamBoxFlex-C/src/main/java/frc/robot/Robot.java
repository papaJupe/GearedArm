// teamBoxFlex-C               Robot.j  cmd/subsys basic format

/* example for testing team geared arm w/ Vortex-Flex motor, 100:1 box,
* CAN bus motor control, built-in motor encoder data used for velocity for
* indexing; Flex methods, eg PIDcontroller not clearly usable now 
*/

/* after deploying, enable teleOp, index arm to full rev. position 0 
 w/ R bumper press; then manually control angle with POV button, or go to
  preset angles by holding buttons A,B,X,Y. Or enable Auto to its sequence
...
* InstantCommand()) -- using classes and methods not needing lambda operator
* [ B  & C now done in this example:]
...
* method to subsys, button press moves to 0 position on start-up (assume limit
* switch stop)
...
import frc.robot.subsystems.ArmFlexSubsys;
import static frc.robot.subsystems.ArmFlexSubsys.*;
...
public class Robot extends TimedRobot {
  ...
  // arm Vortex+Flex motor instanced in this arm subsys
  public final ArmFlexSubsys myArmFlex = new ArmFlexSubsys();

  private Command AutoCommand;
  // private Command teleComd;

  public static double angleGoal = 0;
  ...
  public void robotInit() {
    ...
    // to manually reset encoder to show angle = 0 at current position
    ...
    armForwd.onTrue(new InstantCommand(() -> myArmFlex.armMotorFlex.set(0.3)));
    armForwd.onFalse(new InstantCommand(() -> myArmFlex.armMotorFlex.set(0.0)));
    // both started movement onTrue, didn't stop until .onFalse added,
    armRevrs.onTrue(new InstantCommand(() -> myArmFlex.armMotorFlex.set(-0.3)));
    armRevrs.onFalse(new InstantCommand(() -> myArmFlex.armMotorFlex.set(0.0)));

    // method of encoder superclass; works only when Enabled;
    leftBump.onTrue(new InstantCommand(() -> myArmFlex.absolArmEncod.reset()));
    // move arm to full rev. position, reset absol encoder to 0
    rightBump.onTrue(new InstantCommand(() -> myArmFlex.index()));

    // press x 1 returns arm to home (full reverse) angle, slowly
    buttonA.onTrue( // previous inline PIDcmd format
        new PIDCommand(new PIDController(kP, kI, kD),
            ...
            output -> myArmFlex.armMotorFlex.set(output * 0.1),
            // Require the subsys instance
            myArmFlex))
        .onTrue(new PrintCommand("buttonA press"));
    // .onFalse(new InstantCommand(() -> myArmFlex.armMotorFlex.set(0.0)));
    ...
    buttonB
        .whileTrue(new GoToAngle(myArmFlex, setpointB, 0.15)) // 60deg
        .onFalse(new InstantCommand(() -> myArmFlex.armMotorFlex.set(0.0)))
        .onTrue(new PrintCommand("buttonB press"));

    // go to 90° on buttonX press + hold
    buttonX
        .whileTrue(new GoToAngle(myArmFlex, setpointX, 0.15))
        .onFalse(new InstantCommand(() -> myArmFlex.armMotorFlex.set(0.0)))
        .onTrue(new PrintCommand("buttonX press"));

    buttonY // press & hold --> 120 deg
        .whileTrue(new GoToAngle(myArmFlex, setpointY, 0.15))
        .onFalse(new InstantCommand(() -> myArmFlex.armMotorFlex.set(0.0)))
  ...
  public void robotPeriodic() {
    ...
    // read current PID coeffic from SmtDashbd <-- user changed
    ...
    // kMaxOutput = max;
    // }
    // auto cmd refresh goal
    SmartDashboard.putNumber("angleGoal", angleGoal);
    // display current arm angle
    SmartDashboard.putNumber("armAngle deg.", myArmFlex.getAngle());
  ...
  public void autonomousInit() {
    AutoCommand = new SequentialCommandGroup(
        new GoToAngle(myArmFlex, setpointB, 0.15)
            .andThen(new WaitCommand(2.0))
            .andThen(new GoToAngle(myArmFlex, setpointX, 0.15))
            .andThen(new WaitCommand(2.0))
            .andThen(new GoToAngle(myArmFlex, setpointY, 0.15))
            .andThen(new WaitCommand(2.0))
            .andThen(new GoToAngle(myArmFlex, setpointA, 0.15))
